...
const SaveCouponInfo = () => {
	...
	switch (_action) {
		case 'edit':
			ActionRoute = '/am-admin/intranet/catalogs/edit/' + _id;
			send = catalog;
			break;
		case 'create':
			ActionRoute = '/am-admin/intranet/catalogos';
			catalog.slug = 'intranet-catalogo-' + GenerateSlug(catalog.title);
			send = [ catalog ];
			break;
	}
		...
		$.ajax({
			...
			if(data.post !== undefined && data.post.id !== undefined) {
				...
				switch(_action) {
					...
					break;
					case 'create':
						window.location.href = '/am-admin/intranet/catalogos/' + data.post.id;
					break;
				}
			...
			if(data.posts_id !== undefined && data.posts_id.length > 0) {
			    window.location.href = '/am-admin/intranet/catalogos/' + data.posts_id[0];
			}
		}).catch(function(err) {
